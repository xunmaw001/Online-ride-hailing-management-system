    ...
    <el-tabs tab-position="left" :style='{"border":"2px solid #dbd9f4","padding":"20px","margin":"8px 4px 4px 4px","borderRadius":"16px","background":"#fff"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="80px">
          ...
          <el-form-item :style='{"width":"80%","padding":"10px","margin":"4px auto","background":"none"}' v-if="userTableName=='yonghu'" label="联系电话" prop="lianxidianhua">
          ...
          <el-form-item :style='{"width":"80%","padding":"10px","margin":"4px auto","background":"none"}' v-if="userTableName=='siji'" label="司机账号" prop="sijizhanghao">
            <el-input v-model="sessionForm.sijizhanghao" placeholder="司机账号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"80%","padding":"10px","margin":"4px auto","background":"none"}' v-if="userTableName=='siji'" label="密码" prop="mima">
            <el-input type="password" v-model="sessionForm.mima" placeholder="密码"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"80%","padding":"10px","margin":"4px auto","background":"none"}' v-if="userTableName=='siji'" label="司机姓名" prop="sijixingming">
            <el-input v-model="sessionForm.sijixingming" placeholder="司机姓名" ></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"80%","padding":"10px","margin":"4px auto","background":"none"}' v-if="userTableName=='siji'" label="性别">
            <el-select v-model="sessionForm.xingbie" placeholder="请选择性别" >
              <el-option v-for="(item, index) in dynamicProp.xingbie" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item :style='{"width":"80%","padding":"10px","margin":"4px auto","background":"none"}' v-if="userTableName=='siji'" label="联系方式" prop="lianxifangshi">
            <el-input v-model="sessionForm.lianxifangshi" placeholder="联系方式" ></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"80%","padding":"10px","margin":"4px auto","background":"none"}' v-if="userTableName=='siji'" label="头像">
            <el-upload
              class="avatar-uploader"
              :action="uploadUrl"
              :headers="headers"
              :show-file-list="false"
              :on-success="sijitouxiangHandleAvatarSuccess">
              <img :style='{"border":"1px solid #dbd9f4","cursor":"pointer","color":"#b3aeea","borderRadius":"30px","textAlign":"center","width":"200px","fontSize":"32px","lineHeight":"120px"}' v-if="sessionForm.touxiang" :src="baseUrl + sessionForm.touxiang" class="avatar">
              <i v-else class="el-icon-plus avatar-uploader-icon"></i>
            </el-upload>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if ('siji' == this.userTableName) {
        this.$set(this.sessionForm, 'sijizhanghao', null);
      }
      if ('siji' == this.userTableName) {
        this.$set(this.sessionForm, 'mima', null);
      }
      if ('siji' == this.userTableName) {
        this.$set(this.sessionForm, 'sijixingming', null);
      }
      if ('siji' == this.userTableName) {
        this.$set(this.sessionForm, 'xingbie', null);
      }
      ...
      if ('siji' == this.userTableName) {
        this.$set(this.rules, 'sijizhanghao', [{ required: true, message: '请输入司机账号', trigger: 'blur' }]);
      }
      if ('siji' == this.userTableName) {
        this.$set(this.rules, 'mima', [{ required: true, message: '请输入密码', trigger: 'blur' }]);
      }
      if ('siji' == this.userTableName) {
        this.$set(this.rules, 'sijixingming', [{ required: true, message: '请输入司机姓名', trigger: 'blur' }]);
      }
			if ('siji' == this.userTableName) {
    ...
    methods: {
      ...
      sijitouxiangHandleAvatarSuccess(res, file) {
        if (res.code == 0) {
          if ('siji' == this.userTableName) {
            this.sessionForm.touxiang = 'upload/' + res.file;
          }
